...
This sends documentation-specific events (scroll depth, tab switches, code copies, feedback, etc.) through the same OTel pipeline as your browser auto-instrumentations. Each record is an OTel event: the event name is carried in the standard top-level `eventName` field (serialized as `event_name` on the OTLP wire), alongside `session.id` and other attributes, making it easy to correlate docs behaviour with page load performance, API calls, and errors. If you use `@opentelemetry/browser-sdk` session processors instead, set `sessionAttributes: false` on `DocsInstrumentation` so the `session.id` attribute is not emitted twice.
